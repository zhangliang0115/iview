<style scoped>
    ...
    .ivu-spin-fix{background:none;}
    .mask{background-color: rgba(55,55,55,.6);opacity:0.2}
    .smart-table-page{padding-left: 20px;}
    ...
    <div :class="wrapClasses" :style="styles">
        <div :class="classes">
            <div :class="[prefixCls + '-title']" v-if="showSlotHeader" ref="title">
                <slot name="header">
                    <i-form class="search-form" inline>
                        <Form-item>
                            <i-button class="search-btn" v-for="(button, index) in buttons" :icon="button.icon" :key="button.text" :type="button.type" :disabled="button.disabled" @click="buttonEvent(button,$event)">{{button.text}}</i-button>
                        </Form-item>
                        <Form-item>
                            <i-input class="search-body" v-model="searchValue" @on-enter="handleSubmit" >
                                <Select v-model="searchField" slot="prepend" style="width: 80px" @on-change="onChangeSelect">
                                    <Option v-for="field in searchFields" :key="field.field" :value="field.field">{{field.title}}</Option>
                                </Select>
            ...
            <div :class="[prefixCls + '-body']" :style="bodyStyle" ref="body" @scroll="handleBodyScroll"
                v-show="isFirstLoadReMoteData || !((!!localeNoDataText && (!data || data.length === 0)) || (!!localeNoFilteredDataText && (!rebuildData || rebuildData.length === 0)))">
                <table-body
                    ref="tbody"
                    :prefix-cls="prefixCls"
                    :styleObject="tableStyle"
                ...
                <div v-if="showLoading && !isFirstLoadReMoteData" class="ivu-spin ivu-spin-fix ivu-spin-show-text mask"></div>
            ...
            <div :class="[prefixCls + '-footer']" v-if="showSlotFooter" ref="footer">
                <slot name="footer">
                    <Page class="smart-table-page" :total="total" :current="pageNumber" :page-size="pageSize" :page-size-opts="pageList" size="small" show-elevator show-sizer show-total placement="top"
                          @on-change="onChangeCurrent" @on-page-size-change="onPageSizeChange"></Page>
              </slot>
          </div>
      </div>
  </div>
...
<script>
  ...
  import { oneOf, getStyle, deepCopy, getScrollBarSize,getParameterNames } from '../../utils/assist';
...
//                    style.height = this.scrollBarWidth > 0 ? `${this.bodyHeight}px` : `${this.bodyHeight - 1}px`;
      ...
      methods: {
          ...
          handleSubmit(){
              var keyName=this.searchOp[this.searchField]+this.searchField;
              for(var i in this.searchFields){
                  delete this.queryParams[(this.searchFields[i]['op']||'')+this.searchFields[i]['field']];
              }
              this.queryParams[keyName]=this.searchValue;
              this.load();
          },
...
//                  });
          ...
          checkRow(_index){
              if(this.checkbox && this.checkOnClick){
      ...
      created () {
          ...
          if(this.searchFields.length>0){
                ...
                for(let i=0;i<this.searchFields.length;i++){
                    this.searchOp[this.searchFields[i].field]=this.searchFields[i].op||'';
                }
          }
          this.$nextTick(() => this.ready = true);
          if (!this.context) this.currentContext = this.$parent;
          this.showSlotHeader = (this.$slots.header !== undefined)||this.searchFields.length>0 || this.buttons.length>0;
          this.showSlotFooter = (this.$slots.footer !== undefined)||this.pagination;
